...
Gen123.OC=function(n.rep,pT.true,pE.true,pT.true.c,pE.true.c,rho,beta0,betae,betat,gammad,aw,time.between,A1,A2,A3,ncohort1,cohortsize1,ncohort2,cohortsize2,n3,u00,u11,targetT,targetE,cutoff.eli.T,cutoff.eli.E,
  ...
  N1=6 
  N2=9
  phi=0 ## tunning parameter for RAR
  ...
  phaseIII=function(data2,dose.sel,decision,targetT,cutoff.eli.T){
    ##   return(list( "n.12"=n.12, "y.12"=y.12, "dur.12"=dur.12, "earlystop"=earlystop, "yT.2"=yT.2, "yE.2"=yE.2,"yS.2"=yS.2, "yS.2.obs"=yS.2.obs, "status.2"=status.2, "dose.2"=dose.2, "can.set"=can.set, "follow"=follow   )    )
    ...
    if (decision==0){
      ...
      ## also monitor the toxicity for interim analysis in phase III
      n.123.sel=n.123[dose.sel]
      yT.123.sel=yT.123[dose.sel]
      postT=1-pbeta(targetT,1+yT.123.sel,1+n.123.sel-yT.123.sel)
      T.A=ifelse(postT<cutoff.eli.T,1,0)
      
      if ( (logrank.chisq<=0.54^2)|(T.A==0)  ){  ## early stop and select the control
        final.decision=0
        n.123=c( n.123[(ndose+1)], n.123[1:ndose]   )
        return(list("optimal dose"=dose.sel,"final decision"=final.decision, "patient allocation"=n.123, "toxicity rate"=pT.123, "efficacy rate"=pE.123, "trial duration"=dur.123, "total sample size"=ss.123     ))
      } else if ( (logrank.chisq>2.96^2)&(T.A==1)  ) { ## early stop and select the intervention
        ...
        logrank.chisq=survdiff(Surv(yS.23.obs.comb, status.23.comb) ~ group.23.comb)$chisq
        ## monitor the toxicity at the end of phase III
        n.123.sel=n.123[dose.sel]
        yT.123.sel=yT.123[dose.sel]
        postT=1-pbeta(targetT,1+yT.123.sel,1+n.123.sel-yT.123.sel)
        T.A=ifelse(postT<cutoff.eli.T,1,0)
        
        if ((logrank.chisq>1.94^2)&(T.A==1)){
          final.decision=dose.sel
  ...
  dose.sel.decision.m=rep(0,n.rep) ## dose selection after the go/ no go decision. 
  ...
  for (i in 1: n.rep){
    ...
    data3=phaseIII(data2,dose.sel,decision,targetT,cutoff.eli.T)
    if(dose.sel==0){
      go.m[i]=99
      dose.sel.decision.m[i]=99
    } else{ go.m[i]=decision 
            dose.sel.decision.m[i]=decision*dose.sel
    }
  ...
  return( list("true toxicity"=c(pT.true.all[ndose+1],pT.true.all[1:ndose]), "true efficacy"=c(pE.true.all[ndose+1],pE.true.all[1:ndose]),
               ...
               "go distribution"=table(dose.sel.decision.m[dose.sel.decision.m!=99]  )/length(dose.sel.decision.m[dose.sel.decision.m!=99])       )    )
...
betae=rep(  -log(2),2)
betat=rep(log(1.5),2)
gammad=matrix(c(log( log(0.148)/log(0.3)),log( log(0.148)/log(0.3)),log( log(0.29)/log(0.35)),log( log(0.29)/log(0.35)),log( log(0.66)/log(0.7)),log( log(0.66)/log(0.7)),
